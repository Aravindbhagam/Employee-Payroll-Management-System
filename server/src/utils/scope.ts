 ...
 */
export async function teamEmployeeIds(managerUserId: string): Promise<string[]> {
  const reports = await prisma.user.findMany({
    where: { managerId: managerUserId },
    select: { employee: { select: { id: true } } },
  });
  const ids = reports.map((r) => r.employee?.id).filter((id): id is string => !!id);
 ...
 * their team, Employee sees only themself. Returns `null` for "no filter
 * needed" (full access) or an object with `in: string[]` to constrain to.
 */
export async function employeeScopeFilter(user: AuthUser): Promise<{ in: string[] } | undefined> {
