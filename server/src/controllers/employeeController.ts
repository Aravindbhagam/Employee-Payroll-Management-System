...
function serializeEmployee(emp: any, viewerRole: RoleName, viewerId: string) {
...
export const updateEmployee = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isSelf && user.role === 'EMPLOYEE') {
    data = Object.fromEntries(Object.entries(data).filter(([key]) => SELF_EDITABLE_FIELDS.includes(key))) as typeof data;
  } else if (!isSelf) {
    const scope = await employeeScopeFilter(user);
  ...
  const employee = await prisma.employee.update({
    ...
    data: { ...employeeData, departmentId, designationId, status: status as any },
    include: employeeInclude,
  });
