...
import { createFixtureSet, FixtureSet } from '../helpers/fixtures';
import { bearer, loginAs } from '../helpers/api';

async function setUpSalary(fixtures: FixtureSet, payrollAdminToken: string) {
...
describe('payroll approval workflow', () => {
  ...
  it('walks a run through the full lifecycle: Draft -> Calculating -> Pending Review -> Pending Approval -> Approved -> Processing -> Completed, generating payslips', async () => {
    ...
    const ownPayslip = detail.body.payrollRun.payslips.find((p: any) => p.employeeId === fixtures.employee.employeeId);
