...
const { createFixtureSet, PASSWORD } = require('../helpers/fixtures');
...
describe('auth', () => {
  ...
  it('accepts login by employee code as well as email', async () => {
    ...
    const { rows } = await query('SELECT employee_code AS "employeeCode" FROM users WHERE id = $1', [fixtures.employee.id]);
    const res = await request(app).post('/api/auth/login').send({ identifier: rows[0].employeeCode, password: PASSWORD });
    expect(res.status).toBe(200);
  });
